...
def my_scorer(estimator, x, y):
    yPred = np.sign(estimator.predict(x))
...
def main():# read hard and easy
    ...
    #mkdir()
    classifier = misvm.MISVM(kernel=kernel, gamma=gamma, C=C, verbose=True, max_iters=100)
    #classifier = joblib.load('misvm.pkl.cmp')
    classifier.fit(bags, labels)

    joblib.dump(classifier, path + '/misvm.pkl.cmp', compress=True)
...
def search_hyperparameter():
    global gamma, path
    bags, labels = read_data(method=method, savemotiontemp=True)
    print(len(bags))
    indexes = random.sample([j for j in range(len(bags))], 50)
    ...
    text = '{0}\n'.format(len(bags))
    text_, K, best_g = tune(0.0001, 0.001, 0.0001, train_bags, test_bags)
    text += text_
...
def cross_validation():
    ...
    for i in range(5):
        ...
        with open('{0}/cross_validation_info.txt'.format(path), 'a') as f:
            ...
            classifier = misvm.MISVM(kernel=kernel, gamma=gamma, C=C, verbose=True)
            classifier.fit(train_bags, train_labels)
            predictions = classifier.predict(test_bags)
            joblib.dump(classifier, path + '/misvm{0}.pkl.cmp'.format(i), compress=True)
            # print(test_labels)
    ...
    if check:
        ...
        for cnt, i in enumerate(range(0, len(bags), threadnum)):
            ...
            def work(threadcount):
                ...
                joblib.dump(classifier, path + '/leave-one-out/{0}/misvm.pkl.cmp'.format(i + threadcount),
                            compress=True)
                # print(test_labels)
                # print(np.sign(predictions))
                # print("Accuracy: {0}".format(np.average(test_labels == np.sign(predictions)) * 100))
...
def leave_one_person_out(threadnum=8):
    bags, labels, csvnamelist = read_data(method=method, notnantime=True, savemotiontemp=False)
    ...
    for cnt, i in enumerate(range(0, len(personlist), threadnum)):
        ...
        def work(threadcount, leaveI):
            ...
            classifier.fit(train_bags, train_labels)
            predictions = classifier.predict(train_bags)
            joblib.dump(classifier, path + '/leave-one-person-out/{0}/misvm.pkl.cmp'.format(i + threadcount),
                        compress=True)
            # print(test_labels)
            # print(np.sign(predictions))
            # print("Accuracy: {0}".format(np.average(test_labels == np.sign(predictions)) * 100))
...
def result_leave_one_person_out(bags=None, labels=None):
    ...
    for dir in dirs:
        ...
        classifier = joblib.load(dir + 'misvm.pkl.cmp')
...
def get_from_openpose():
    op = OpenPose()
    #op.get_from_openpose(videosdir='/home/junkado/Desktop/keio/hard/cut-video')
    op.manual_videofile("/home/junkado/Desktop/keio/hard/cut-video/C1105.MP4")
...
if __name__ == '__main__':
    ...
    CC = [500, 1000, 2500, 5000, 7500]
    for cc in CC:
        path = './result/MISVM/{0}/g{1}c{2}'.format(dir_name, gamma, cc)
        C = cc
